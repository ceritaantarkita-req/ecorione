...
describe("MCP handle AEAD", () => {
  ...
  it("requestState terikat principal + digest request", () => {
    const request = {
      method: "tools/call",
      params: { name: "memory_propose", arguments: { text: "x" } },
    };
    const token = sealRequestState(KEY, "alice", request, { step: 1 }, NOW, 10_000);
    expect(openRequestState<{ step: number }>(KEY, token, "alice", request, NOW + 1)).toEqual({
      step: 1,
    });
    expect(() =>
      openRequestState(KEY, token, "alice", { ...request, method: "tools/list" }, NOW + 1),
    ).toThrow(/request/i);
  });
});
