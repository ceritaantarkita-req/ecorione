...
import { JwksCache, McpAuthError, authenticateBearer, requireOAuthScope, type McpAuthConfig } from "./auth.js";

const { privateKey, publicKey } = generateKeyPairSync("rsa", { modulusLength: 2048 });
const PUBLIC_JWK = { ...publicKey.export({ format: "jwk" }), kid: "k1", alg: "RS256", use: "sig" };
...
describe("MCP OAuth resource server", () => {
  it("memvalidasi signature + issuer + audience/resource + scopes", async () => {
    const cfg = config();
    const principal = await authenticateBearer(`Bearer ${jwt()}`, cfg, new JwksCache(cfg), NOW_MS);
  ...
  it("menolak token dengan audience lain (anti token passthrough)", async () => {
    const cfg = config();
    await expect(authenticateBearer(`Bearer ${jwt({ aud: "https://other.example" })}`, cfg, new JwksCache(cfg), NOW_MS))
      .rejects.toBeInstanceOf(McpAuthError);
  });

  it("scope OAuth write tidak bisa dipalsukan dari memory scope", async () => {
    const cfg = config();
    const principal = await authenticateBearer(`Bearer ${jwt({ scope: "memory:read" })}`, cfg, new JwksCache(cfg), NOW_MS);
