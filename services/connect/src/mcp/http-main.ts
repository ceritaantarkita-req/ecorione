...
if (host !== "127.0.0.1") {
  throw new Error(
    "ECORIONE_MCP_HOST wajib 127.0.0.1; exposure publik hanya lewat Sync/tunnel (ADR-16).",
  );
}
...
if (issuer === undefined || resource === undefined || jwksUrl === undefined) {
  throw new Error(
    "MCP HTTP butuh ECORIONE_MCP_OAUTH_ISSUER, ECORIONE_MCP_RESOURCE, dan ECORIONE_MCP_JWKS_URL.",
  );
}
if (handleKeyRaw === undefined) {
  throw new Error(
    "MCP HTTP butuh ECORIONE_MCP_HANDLE_KEY base64url 32-byte agar handle tetap valid selama restart.",
  );
}
const defaultMemoryScopes = z
  .array(ScopeSchema)
  .min(1)
  .parse(
...
const defaultMaxSensitivity = SensitivitySchema.parse(
  process.env.ECORIONE_MCP_MAX_SENSITIVITY ?? "INTERNAL",
);
