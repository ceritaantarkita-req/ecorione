...
export function validateModernEnvelope(request: McpRequest): void {
  ...
  if (typeof meta !== "object" || meta === null) {
    throw new McpProtocolError(
      -32602,
      `MCP ${MCP_PROTOCOL_VERSION} membutuhkan params._meta per request.`,
    );
  }
  const parsed = EnvelopeSchema.safeParse(meta);
  if (!parsed.success) {
    throw new McpProtocolError(
      -32602,
      `Envelope MCP ${MCP_PROTOCOL_VERSION} tidak valid.`,
      parsed.error.issues,
    );
  }
}
...
export function validateHttpRoutingHeaders(
  ...
  if (request.method === "tools/call") {
    ...
    if (headers.name !== params.name)
      throw new McpProtocolError(
        -32020,
        "Mcp-Name header tidak cocok dengan tools/call params.name.",
      );
  } else if (headers.name !== undefined) {
    throw new McpProtocolError(
      -32020,
      "Mcp-Name hanya dikirim untuk method yang memiliki subject/name.",
    );
  }
}
...
function success(
  ...
  return {
    ...
    result: {
      ...result,
      _meta: { ...(result._meta as Record<string, unknown> | undefined), ...responseMeta() },
    },
  };
}
...
export function errorResponse(
  ...
  const protocol =
    error instanceof McpProtocolError
      ? error
      : new McpProtocolError(-32603, error instanceof Error ? error.message : String(error));
