...
describe("Connect operations telemetry", () => {
  it("menjalankan local provider canary melalui completion boundary dan merekam metrics", async () => {
    ...
    if (address === null || typeof address === "string") throw new Error("fake local address gagal");
    ...
    const counters = (metrics.json() as { counters: Array<{ name: string; value: number }> }).counters;
    expect(counters.some((item) => item.name === "ecorione_provider_canary_total" && item.value === 1)).toBe(true);
    expect(counters.some((item) => item.name === "ecorione_model_input_tokens_total" && item.value === 7)).toBe(true);
  });
