...
const dbPath = process.env.ECORIONE_FLOW_DB ?? resolve(import.meta.dirname, "../../../data/flow.sqlite");
const db = openFlowDatabase(dbPath);
const temporal = await createFlowTemporalClient({ address: temporalAddress, namespace: temporalNamespace });
const app = buildFlowServer(temporal, { hubUrl, token, logger: true, graphRepository: new FlowGraphRepository(db) });
app.addHook("onClose", async () => db.close());
app.listen({ port, host: bindHost() }).then(() => app.log.info(`Flow jalan di http://${bindHost()}:${String(port)}`)).catch((err: unknown) => { app.log.error(err); db.close(); process.exit(1); });
