...
import { BadRequestError, ConflictError, HttpError, NotFoundError, createServer, httpJson, parseOrBadRequest } from "@ecorione/shared-server";
import type { FastifyInstance } from "fastify";
import { z } from "zod";
import { nowIso } from "./clock.js";
import { openFlowDatabase } from "./db.js";
import { FlowGraphNotFoundError, FlowGraphRepository, FlowGraphVersionConflictError, FlowGraphWorkspaceConflictError } from "./graph-repository.js";
import { digestCanonical, listCoreNodeDefinitions, validateAndCompileFlowGraph } from "./node-registry.js";
...
const GraphVersionQuerySchema = z.object({ version: z.coerce.number().int().min(1).optional() });
...
function graphError(error: unknown): unknown {
  ...
  if (error instanceof FlowGraphVersionConflictError || error instanceof FlowGraphWorkspaceConflictError) return new ConflictError(error.message);
  return error;
}
function requireGraphTemporal(temporal: FlowServerTemporalClient): FlowGraphTemporalClient {
  if (temporal.startGraph === undefined || temporal.signalGraphDecision === undefined || temporal.signalGraphInput === undefined || temporal.graphState === undefined) {
    throw new HttpError(503, "FLOW_GRAPH_RUNTIME_UNAVAILABLE", "Temporal graph runtime tidak dikonfigurasi.");
  }
...
async function syncNodeDeclarations(options: BuildFlowServerOptions, workspaceId: string): Promise<void> {
  await httpJson(`${options.hubUrl}/v1/authority/nodes/sync`, { method: "POST", token: options.token, body: { workspaceId, definitions: listCoreNodeDefinitions() } });
}

export function buildFlowServer(temporal: FlowServerTemporalClient, options: BuildFlowServerOptions): FastifyInstance {
  ...
  app.post("/v1/flows", async (req, reply) => {
    ...
    return reply.code(202).send(FlowStartResponseSchema.parse({ flowId, operationId, temporalWorkflowId: flowId }));
  });
  ...
  app.post<{ Params: { id: string } }>("/v1/flows/:id/decision", async (req) => {
    ...
    const approval = await httpJson<{ operationId: OperationId }>(`${options.hubUrl}/v1/approvals/by-idempotency-key?idempotencyKey=${encodeURIComponent(approvalKey)}`, { token: options.token });
    const operationId = assertId("operation", approval.operationId);
    const hubBody: { decision: "APPROVE" | "REJECT"; note?: string } = { decision: body.decision };
    if (body.note !== null) hubBody.note = body.note;
    await httpJson(`${options.hubUrl}/v1/approvals/${operationId}/decide`, { method: "POST", token: options.token, body: hubBody });
  ...
  app.get<{ Params: { id: string } }>("/v1/graphs/:id", async (req) => {
    ...
    try { return graphs.get(id, query.version); } catch (error) { throw graphError(error); }
  });
  ...
  app.put<{ Params: { id: string } }>("/v1/graphs/:id", async (req) => {
    ...
    try { return graphs.save(graph, expectedVersion, nowIso() as Timestamp); } catch (error) { throw graphError(error); }
  });

  app.get<{ Params: { id: string } }>("/v1/graphs/:id/versions", async (req) => {
    const { id } = parseOrBadRequest(GraphParamsSchema, req.params);
    try { return { versions: graphs.versions(id) }; } catch (error) { throw graphError(error); }
  });
  ...
  app.post<{ Params: { id: string } }>("/v1/graphs/:id/runs", async (req, reply) => {
    ...
    let graphVersion;
    try { graphVersion = graphs.get(id, body.version); } catch (error) { throw graphError(error); }
    if (!graphVersion.validation.valid || graphVersion.validation.plan === null) throw new BadRequestError("Graph version tidak valid dan tidak dapat dijalankan.");
    const graphTemporal = requireGraphTemporal(temporal);
    const runId = makeId("workflow");
    const operationId = makeId("operation");
    await graphTemporal.startGraph({ runId, operationId, plan: graphVersion.validation.plan, input: body.input, depth: 0 });
    return reply.code(202).send(FlowGraphRunResponseSchema.parse({ runId, graphId: id, graphVersion: graphVersion.version, temporalWorkflowId: runId, traceOperationId: operationId, planDigest: graphVersion.validation.plan.planDigest }));
  });

  app.get<{ Params: { id: string } }>("/v1/graph-runs/:id", async (req) => {
    const { id } = parseOrBadRequest(RunParamsSchema, req.params);
    const graphTemporal = requireGraphTemporal(temporal);
    const [description, state] = await Promise.all([temporal.describe(id), graphTemporal.graphState(id)]);
    return { temporalStatus: description.status, ...state };
  });

  app.post<{ Params: { id: string; nodeId: string } }>("/v1/graph-runs/:id/nodes/:nodeId/decision", async (req) => {
    ...
    if (!validKeys.has(body.approvalKey)) throw new BadRequestError("approvalKey tidak cocok dengan run/node yang dituju.");
    const approval = await httpJson<{ operationId: OperationId }>(`${options.hubUrl}/v1/approvals/by-idempotency-key?idempotencyKey=${encodeURIComponent(body.approvalKey)}`, { token: options.token });
    const operationId = assertId("operation", approval.operationId);
    const hubBody: { decision: "APPROVE" | "REJECT"; note?: string } = { decision: body.decision };
    if (body.note !== null) hubBody.note = body.note;
    await httpJson(`${options.hubUrl}/v1/approvals/${operationId}/decide`, { method: "POST", token: options.token, body: hubBody });
    await requireGraphTemporal(temporal).signalGraphDecision(id, { nodeId, decision: body.decision, note: body.note });
    return { runId: id, nodeId, operationId, decision: body.decision };
  });

  app.post<{ Params: { id: string; nodeId: string } }>("/v1/graph-runs/:id/nodes/:nodeId/input", async (req) => {
    const { id, nodeId } = parseOrBadRequest(NodeRunParamsSchema, req.params);
