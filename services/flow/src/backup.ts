...
function assertHealthy(db: FlowDatabase): void {
  ...
  if (quick !== "ok") throw new BackupIntegrityError(`Flow quick_check gagal: ${String(quick)}`);
...
export async function backupFlowGraphRegistry(db: FlowDatabase, backupRoot: string, createdAt: string): Promise<BackupManifest> {
  assertHealthy(db);
  const store = new OwnerBackupStore(backupRoot, "flow");
  const release = store.acquireOperationLock();
  try {
    return await store.createSqlite("flow-graph-registry", (destination) => db.raw.backup(destination), createdAt);
  } finally {
    release();
  }
}
...
export function restoreFlowGraphRegistry(backupRoot: string, backupId: string, targetPath: string, restoredAt: string): RestoreReceipt {
  const store = new OwnerBackupStore(backupRoot, "flow");
  const release = store.acquireOperationLock();
  try {
