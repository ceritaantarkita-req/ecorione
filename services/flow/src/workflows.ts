...
export const graphNodeDecisionSignal =
...
function nodeInput(
  execution: FlowGraphExecutionInput,
  nodeId: FlowNodeId,
  outputs: Map<string, NodeOutput>,
  statuses: Map<string, string>,
): { active: boolean; value: unknown } {
...
export async function graphExecutionWorkflow(
  ...
  const statuses = new Map<string, string>(
    execution.plan.nodes.map((item) => [item.node.id, "PENDING"]),
  );
  const approvalKeys = new Map<string, string | null>(
    execution.plan.nodes.map((item) => [item.node.id, null]),
  );
  ...
  const state = (): FlowGraphRunState => ({
    ...
    status: runStatus,
    nodes: execution.plan.nodes.map((item) => ({
      nodeId: item.node.id,
      status: statuses.get(item.node.id) as FlowGraphRunState["nodes"][number]["status"],
      approvalKey: approvalKeys.get(item.node.id) ?? null,
      message: messages.get(item.node.id) ?? null,
    })),
    output: finalOutput,
    error: finalError,
  });
  ...
  setHandler(graphNodeDecisionSignal, (signal) => {
    decisions.set(signal.nodeId, signal);
  });
  ...
  async function executeNode(compiled: FlowCompiledNode): Promise<void> {
    ...
    try {
      ...
      await dynamic.recordGraphTrace({
        execution,
        compiled,
        name: "flow.graph.node.started",
        attributes: { kind: node.kind },
      });
      const policyKey = `${execution.runId}:${node.id}:policy`;
      if (node.kind !== "approval" && compiled.actionClass !== null) {
        const verdict = await dynamic.evaluateGraphNodePolicy({
          execution,
          compiled,
          approvalKey: policyKey,
        });
        if (verdict.outcome === "DENY")
          throw ApplicationFailure.nonRetryable(
            `Node policy ditolak: ${verdict.reason}`,
            "FLOW_NODE_POLICY_DENIED",
          );
      ...
      if (node.kind === "trigger") output = { value: execution.input, route: null };
        ...
        output = { value: transformGraphValue(incoming.value, node.config), route: null };
      else if (node.kind === "condition")
        output = {
          value: incoming.value,
          route: conditionGraphRoute(incoming.value, node.config),
        };
      else if (node.kind === "loop")
        output = { value: loopGraphValue(incoming.value, node.config), route: null };
        ...
        const prompt = await dynamic.requestGraphApproval({
          execution,
          compiled,
          approvalKey,
          prompt: cfg.prompt,
        ...
        const decision = decisions.get(node.id);
        if (decision?.decision !== "APPROVE")
          throw ApplicationFailure.nonRetryable("Approval node ditolak.", "FLOW_NODE_REJECTED");
        approvalKeys.set(node.id, null);
        messages.set(node.id, null);
        statuses.set(node.id, "RUNNING");
        output = { value: incoming.value, route: null };
        ...
        if (execution.depth >= 8)
          throw ApplicationFailure.nonRetryable(
            "Subflow depth mencapai batas 8.",
            "FLOW_SUBFLOW_DEPTH",
          );
        const plan = await dynamic.resolveSubflow({ execution, compiled });
        if (plan.graph.id === execution.plan.graph.id)
          throw ApplicationFailure.nonRetryable(
            "Direct recursive subflow ditolak.",
        ...
        output = {
          value: await dynamic.executeGraphNode({ execution, compiled, input: incoming.value }),
          route: null,
        };
      }
      ...
      await dynamic.recordGraphTrace({
        execution,
        compiled,
        name: "flow.graph.node.completed",
        attributes: { kind: node.kind },
      });
    } catch (error) {
      ...
      try {
        await dynamic.recordGraphTrace({
          execution,
          compiled,
          name: "flow.graph.node.failed",
          attributes: { kind: node.kind, error: message },
        });
      } catch {
  ...
  try {
    for (const level of execution.plan.levels) {
      const ordered = level
        .map((id) => byId.get(id))
        .filter((item): item is FlowCompiledNode => item !== undefined);
      for (
        let index = 0;
        index < ordered.length;
        index += execution.plan.graph.maxParallelism
      ) {
        await Promise.all(
          ordered.slice(index, index + execution.plan.graph.maxParallelism).map(executeNode),
        );
      }
    }
    const outgoing = new Set(execution.plan.graph.edges.map((edge) => edge.sourceNodeId));
    const sinks = execution.plan.nodes.filter(
      (item) => !outgoing.has(item.node.id) && statuses.get(item.node.id) === "SUCCEEDED",
    );
