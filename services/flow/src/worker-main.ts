...
const csv = (value: string | undefined): string[] => value?.split(",").map((item) => item.trim()).filter((item) => item.length > 0) ?? [];
const worker = await createFlowWorker({
  temporalAddress, temporalNamespace, hubUrl, connectUrl, contextUrl, artifactUrl, spaceUrl, sandboxUrl, rndUrl, flowUrl, token,
...
if (process.env.ECORIONE_FLOW_WORKER_READY_IPC === "1" && process.send !== undefined) {
  const ready = (async () => {
    for (let attempt = 0; attempt < 1500; attempt += 1) {
      ...
      if (state === "STOPPING" || state === "DRAINING" || state === "DRAINED" || state === "STOPPED") throw new Error(`Flow worker stopped before readiness: ${state}`);
      await new Promise<void>((resolve) => setTimeout(resolve, 10));
    }
    throw new Error(`Timed out waiting for Flow worker RUNNING state; current=${worker.getState()}`);
  })();
  await Promise.race([ready, runPromise.then(() => { throw new Error("Flow worker run loop stopped before readiness."); }, (error: unknown) => Promise.reject(error))]);
