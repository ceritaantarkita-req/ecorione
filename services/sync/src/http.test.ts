...
async function setupPair() {
  ...
  const app = buildSyncServer(db, {
    ownerToken: OWNER,
    connectMcpUrl: "http://127.0.0.1:65535",
    clock: () => NOW,
  });
...
describe("Sync device relay", () => {
  it("bootstrap membutuhkan owner token dan hanya bisa sekali", async () => {
    ...
    const denied = await app.inject({
      method: "POST",
      url: "/v1/devices/bootstrap",
      payload: { name: "x", publicKey: key.publicKey },
    });
    ...
    const second = await app.inject({
      ...
      url: "/v1/devices/bootstrap",
      headers: { "x-ecorione-owner-token": OWNER },
      payload: { name: "y", publicKey: key.publicKey },
    });
  ...
  it("pairing code sekali pakai", async () => {
    ...
    const replay = await app.inject({
      method: "POST",
      url: "/v1/devices/pair",
      payload: { code, name: "replay", publicKey: b.publicKey },
    });
  ...
  it("relay menyimpan ciphertext saja dan recipient dapat decrypt E2E", async () => {
    ...
    const stored = db?.raw.prepare("SELECT ciphertext FROM relay_messages").get() as {
      ciphertext: string;
    };
    ...
    const parsed = JSON.parse(inbox.body) as {
      messages: Array<{
        id: string;
