...
export class QuarantineRequiredError extends ContextError {
  constructor(trust: Trust) { super(`Trust "${trust}" harus lewat proposeFact() → karantina, bukan insertFact() (ADR-07).`); }
}
export class QuarantineStateError extends ContextError {}
export class FactNotFoundError extends ContextError { constructor(id: MemoryFactId) { super(`Fakta tidak ditemukan: ${id}`); } }
export class FactAlreadyInvalidatedError extends ContextError { constructor(id: MemoryFactId) { super(`Fakta ${id} sudah di-invalidate — supersede rantai dari fakta penggantinya.`); } }
export class FactDeletionForbiddenError extends ContextError {
  constructor() { super("Fakta tidak pernah dihapus (ADR-06). Pakai forgetFact(), supersedeFact(), atau rebuildDerivedTiers()."); }
}
export class CoreMemoryLimitError extends ContextError {
  constructor(chars: number) { super(`Memori inti ${String(chars)} karakter, melewati batas ~${String(CORE_MEMORY_TOKEN_LIMIT)} token (${String(CORE_MEMORY_CHAR_LIMIT)} karakter).`); }
}
export class CoreMemoryWriteForbiddenError extends ContextError {}
export class ScopeEscalationError extends ContextError {
  constructor(from: Scope, to: Scope) { super(`Promosi karantina tidak boleh memindahkan scope: usulan "${from}" → fakta "${to}".`); }
}

export type AppendEpisodeInput = z.input<typeof EpisodeSchema>;
export type InsertFactInput = z.input<typeof MemoryFactSchema>;
export type ProposeFactInput = Omit<z.input<typeof QuarantinedWriteSchema>, "status" | "rejectionReason" | "reviewedAt">;
...
export interface FactLookupOptions { readonly includeInvalidated?: boolean | undefined; }
...
export interface SupersedeResult { readonly invalidated: MemoryFact; readonly replacement: MemoryFact; }
export interface CoreMemoryWriteOptions { readonly trust: Trust; }
...
export class ContextRepository {
  ...
  constructor(private readonly database: ContextDatabase, private readonly vectors?: VectorIndex | undefined) { this.raw = database.raw; }
  get db(): ContextDatabase { return this.database; }
  get vectorIndex(): VectorIndex | undefined { return this.vectors; }

  appendEpisode(input: AppendEpisodeInput): Episode {
    const episode = EpisodeSchema.parse(input);
    this.raw.prepare(`INSERT INTO episodes (
      ...
      id: episode.id, ts: episode.ts, raw_text: episode.rawText,
      ...provenanceParams(episode.provenance), scope: episode.scope, sensitivity: episode.sensitivity,
      sync_class: episode.syncClass, trust: episode.trust, summary: episode.summary, consolidated_at: episode.consolidatedAt,
    });
    return episode;
  }
  getEpisode(id: EpisodeId): Episode | null {
    const row = this.raw.prepare("SELECT * FROM episodes WHERE id = ?").get(id) as EpisodeRow | undefined;
    return row === undefined ? null : rowToEpisode(row);
  }
  listEpisodes(filter: ListEpisodesFilter = {}): Episode[] {
    const clauses: string[] = [];
    const params: (string | number)[] = [];
    if (filter.scopes !== undefined && filter.scopes.length > 0) { clauses.push(`scope IN (${placeholders(filter.scopes.length)})`); params.push(...filter.scopes); }
    if (filter.sessionId !== undefined) { clauses.push("session_id = ?"); params.push(filter.sessionId); }
    if (filter.since !== undefined) { clauses.push("ts >= ?"); params.push(filter.since); }
    if (filter.onlyUnconsolidated === true) clauses.push("consolidated_at IS NULL");
    if (filter.hostedEligibleOnly === true) clauses.push("sync_class IN ('CLOUD_ALLOWED','PUBLIC')");
    const where = clauses.length > 0 ? `WHERE ${clauses.join(" AND ")}` : "";
    const direction = filter.order === "desc" ? "DESC" : "ASC";
    const rows = this.raw.prepare(`SELECT * FROM episodes ${where} ORDER BY ts ${direction}, id ${direction} LIMIT ?`)
  ...
  markEpisodeConsolidated(id: EpisodeId, summary: string | null, now: Timestamp): void {
    this.raw.prepare("UPDATE episodes SET summary = ?, consolidated_at = ? WHERE id = ?").run(summary, now, id);
  }

  proposeFact(input: ProposeFactInput): MemoryFactId {
    const p = QuarantinedWriteSchema.parse({ ...input, status: "PENDING" });
    this.raw.prepare(`INSERT INTO quarantine (
    ...
    )`).run({ id: p.id, proposed_text: p.proposedText, proposed_at: p.proposedAt, ...provenanceParams(p.provenance), trust: p.trust, scope: p.scope });
    return p.id;
  }
  getQuarantined(id: MemoryFactId): QuarantinedWrite | null {
    const row = this.raw.prepare("SELECT * FROM quarantine WHERE id = ?").get(id) as QuarantineRow | undefined;
    return row === undefined ? null : rowToQuarantined(row);
  }
  listQuarantine(status: QuarantinedWrite["status"] = "PENDING"): QuarantinedWrite[] {
    return (this.raw.prepare("SELECT * FROM quarantine WHERE status = ? ORDER BY proposed_at ASC, id ASC").all(status) as QuarantineRow[]).map(rowToQuarantined);
  }
  promoteFromQuarantine(
    ...
    if (proposal === null) throw new QuarantineStateError(`Usulan karantina tidak ditemukan: ${quarantineId}`);
    if (proposal.status !== "PENDING") throw new QuarantineStateError(`Usulan ${quarantineId} sudah berstatus ${proposal.status}.`);
    const fact = MemoryFactSchema.parse(factInput);
    if (fact.scope !== proposal.scope) throw new ScopeEscalationError(proposal.scope, fact.scope);
    ...
    if (options.supersedes !== undefined) {
      ...
      if (Date.parse(fact.tValid) < Date.parse(old.tValid)) throw new ContextError("t_valid pengganti mendahului fakta lama.");
    }
    return this.raw.transaction(() => {
      this.insertFactRow(fact);
      if (old !== null) this.raw.prepare("UPDATE facts SET t_invalid = ?, superseded_by = ? WHERE id = ?").run(fact.tValid, fact.id, old.id);
      this.raw.prepare("UPDATE quarantine SET status='PROMOTED', reviewed_at=?, promoted_fact_id=?, rejection_reason=NULL WHERE id=?").run(now, fact.id, quarantineId);
      return fact;
    })();
  }
  rejectQuarantined(id: MemoryFactId, reason: string, now: Timestamp): void {
    const changes = this.raw.prepare("UPDATE quarantine SET status='REJECTED', rejection_reason=?, reviewed_at=? WHERE id=? AND status='PENDING'").run(reason, now, id).changes;
    if (changes === 0) throw new QuarantineStateError(`Tidak ada usulan PENDING dengan id ${id} untuk ditolak.`);
  }

  private insertFactRow(fact: MemoryFact): void {
    this.raw.prepare(`INSERT INTO facts (
  ...
  private factRow(id: MemoryFactId, includeInvalidated: boolean): FactRow | undefined {
    ...
    return this.raw.prepare(`SELECT ${FACT_COLUMNS} FROM facts WHERE id = ? ${live}`).get(id) as FactRow | undefined;
  }
  ...
  getFactsByIds(ids: readonly MemoryFactId[], options: FactLookupOptions = {}): Map<MemoryFactId, MemoryFact> {
    const result = new Map<MemoryFactId, MemoryFact>();
    if (ids.length === 0) return result;
    const live = (options.includeInvalidated ?? false) ? "" : "AND t_invalid IS NULL";
    const rows = this.raw.prepare(`SELECT ${FACT_COLUMNS} FROM facts WHERE id IN (${placeholders(ids.length)}) ${live}`).all(...ids) as FactRow[];
    for (const row of rows) { const fact = rowToFact(row); result.set(fact.id, fact); }
    return result;
  }
  listFacts(filter: ListFactsFilter = {}): MemoryFact[] {
    ...
    if (filter.scopes !== undefined && filter.scopes.length > 0) { clauses.push(`scope IN (${placeholders(filter.scopes.length)})`); params.push(...filter.scopes); }
    if (filter.subject !== undefined) { clauses.push("subject = ?"); params.push(filter.subject); }
    ...
    if (filter.hostedEligibleOnly === true) clauses.push("sync_class IN ('CLOUD_ALLOWED','PUBLIC')");
    const where = clauses.length > 0 ? `WHERE ${clauses.join(" AND ")}` : "";
    return (this.raw.prepare(`SELECT ${FACT_COLUMNS} FROM facts ${where} ORDER BY created_at DESC, id ASC LIMIT ?`).all(...params, filter.limit ?? 200) as FactRow[]).map(rowToFact);
  }
  supersedeFact(oldId: MemoryFactId, newFactInput: InsertFactInput): SupersedeResult {
    ...
    if (Date.parse(replacement.tValid) < Date.parse(old.tValid)) throw new ContextError("t_valid pengganti mendahului fakta lama.");
    return this.raw.transaction(() => {
      this.insertFactRow(replacement);
      this.raw.prepare("UPDATE facts SET t_invalid=?, superseded_by=? WHERE id=?").run(replacement.tValid, replacement.id, oldId);
  ...
  forgetFact(id: MemoryFactId, now: Timestamp): MemoryFact {
    ...
    if (Date.parse(now) < Date.parse(fact.tValid)) throw new ContextError(`Waktu forget (${now}) mendahului t_valid fakta (${fact.tValid}).`);
  ...
  setSalience(id: MemoryFactId, salience: number): void {
    if (salience < 0 || salience > 1) throw new ContextError(`Salience harus di 0..1, diterima ${String(salience)}.`);
    if (this.raw.prepare("UPDATE facts SET salience=? WHERE id=?").run(salience, id).changes === 0) throw new FactNotFoundError(id);
  }
  deleteFact(_id: MemoryFactId): never { throw new FactDeletionForbiddenError(); }
  ...
  rebuildDerivedTiers<T>(rebuild: (repo: this) => T): T {
    return this.raw.transaction(() => {
      this.raw.prepare("INSERT INTO context_meta (key,value) VALUES ('allow_derived_rebuild','1') ON CONFLICT(key) DO UPDATE SET value='1'").run();
      try {
        this.raw.prepare("UPDATE quarantine SET promoted_fact_id = NULL WHERE promoted_fact_id IS NOT NULL").run();
  ...
  putFactEmbedding(factId: MemoryFactId, embedding: Float32Array): void {
    if (this.vectors === undefined) throw new ContextError("Repository dibuat tanpa VectorIndex — embedding tidak bisa ditulis.");
  ...
  getCoreMemory(filter: CoreMemoryFilter = {}): CoreMemory {
    ...
    if (filter.hostedEligibleOnly === true) clauses.push("sync_class IN ('CLOUD_ALLOWED','PUBLIC')");
    const where = clauses.length > 0 ? `WHERE ${clauses.join(" AND ")}` : "";
    const rows = this.raw.prepare(`SELECT * FROM core_memory ${where} ORDER BY label ASC`).all(...params) as CoreMemoryRow[];
    return CoreMemorySchema.parse({ blocks: rows.map(rowToCoreBlock) });
  }
  getCoreMemoryBlock(label: string): CoreMemoryBlock | null {
    const row = this.raw.prepare("SELECT * FROM core_memory WHERE label=?").get(label) as CoreMemoryRow | undefined;
    return row === undefined ? null : rowToCoreBlock(row);
  }
  setCoreMemoryBlock(input: CoreMemoryBlockInput, options: CoreMemoryWriteOptions): CoreMemoryBlock {
    ...
    if (!mayWriteCoreMemory(options.trust)) {
      if (options.trust !== "LOCAL_AGENT") throw new CoreMemoryWriteForbiddenError(`Trust "${options.trust}" tidak boleh menulis memori inti.`);
      if (existing?.readOnly === true || block.readOnly) throw new CoreMemoryWriteForbiddenError(`Blok "${block.label}" read-only — hanya pengguna yang boleh menulisnya.`);
    }
    const others = this.raw.prepare("SELECT COALESCE(SUM(LENGTH(value)),0) AS n FROM core_memory WHERE label <> ?").get(block.label) as { n: number };
    ...
    this.raw.prepare(`INSERT INTO core_memory (label,description,value,read_only,updated_at,scope,sensitivity,sync_class,trust)
      ...
      .run({ label: block.label, description: block.description, value: block.value, read_only: block.readOnly ? 1 : 0,
        updated_at: block.updatedAt, scope, sensitivity, sync_class: syncClass, trust });
  ...
  deleteCoreMemoryBlock(label: string, options: CoreMemoryWriteOptions): void {
    ...
    if (existing.readOnly && !mayWriteCoreMemory(options.trust)) throw new CoreMemoryWriteForbiddenError(`Blok "${label}" read-only.`);
  ...
  putArtifactPointer(input: ArtifactPointerInput): ArtifactPointer {
    const p = ArtifactPointerSchema.parse({ ...input, syncClass: input.syncClass ?? "LOCAL_ONLY" });
    const syncClass = p.syncClass ?? "LOCAL_ONLY";
    this.raw.prepare(`INSERT INTO artifact_pointers (id,path,description,mime_type,size_bytes,scope,sensitivity,sync_class)
      ...
      .run({ id: p.id, path: p.path, description: p.description, mime_type: p.mimeType, size_bytes: p.sizeBytes,
        scope: p.scope, sensitivity: p.sensitivity, sync_class: syncClass });
    return { ...p, syncClass };
  }
  listArtifactPointers(scopes: readonly Scope[], max: Sensitivity = "RESTRICTED", hostedEligibleOnly = false): ArtifactPointer[] {
    if (scopes.length === 0) return [];
    const allowed = allowedSensitivities(max);
    const egress = hostedEligibleOnly ? " AND sync_class IN ('CLOUD_ALLOWED','PUBLIC')" : "";
    const rows = this.raw.prepare(`SELECT * FROM artifact_pointers WHERE scope IN (${placeholders(scopes.length)}) AND sensitivity IN (${placeholders(allowed.length)})${egress} ORDER BY id ASC`)
