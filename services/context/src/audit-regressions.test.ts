...
describe("audit regressions — Context", () => {
  it("FTS menerapkan scope sebelum bm25 LIMIT sehingga kandidat scope lain tidak bisa menenggelamkan hasil sah", () => {
    ...
    for (let i = 0; i < 30; i += 1) {
      repo.insertFact(
        factInput({
          id: `mem_work_${i}`,
          scope: "work",
          text: `needle needle needle work ${i}`,
          object: `work ${i}`,
        }),
      );
    }
    repo.insertFact(
      factInput({
        id: "mem_personal",
        scope: "personal",
        text: "needle personal",
        object: "personal",
      }),
    );
    const result = new ContextRetriever(repo).retrieve({
      query: "needle",
      scopes: ["personal"],
      now: NOW,
    });
    ...
    for (let i = 0; i < 10; i += 1) {
      repo.appendEpisode(
        episodeInput({
          id: `epi_other_${i}`,
          ts: T1,
          provenance: { sourceApp: "ai", sessionId: "sess_other" },
        }),
      );
    }
    repo.appendEpisode(
      episodeInput({
        id: "epi_target_old",
        ts: T0,
        provenance: { sourceApp: "ai", sessionId: "sess_target" },
      }),
    );
    repo.appendEpisode(
      episodeInput({
  ...
  it("mixed-scope consolidation mempertahankan sourceEpisodeId, scope, sensitivity, dan syncClass per kandidat", async () => {
    ...
    repo.appendEpisode(
      episodeInput({
        id: "epi_work",
        rawText: "Proyek Atlas deadline Jumat",
        scope: "work",
        sensitivity: "SENSITIVE",
        syncClass: "LOCAL_ONLY",
        provenance: { sourceApp: "ai", sessionId: "sess_work" },
      }),
    );
    const extractLocal = vi.fn().mockResolvedValue(
      JSON.stringify({
        facts: [
          ...
          {
            sourceEpisodeId: "epi_work",
            subject: "Proyek Atlas",
            predicate: "deadline",
            object: "Jumat",
            confidence: 0.9,
            worthRemembering: true,
  ...
  it("rebuild L1 setelah proposal promoted tidak gagal FK dan tidak menghapus L2", () => {
    ...
    repo.setCoreMemoryBlock(
      { label: "persona", description: "identitas", value: "Rio", updatedAt: T0 },
      { trust: "USER" },
    );
    const id = repo.proposeFact({
      id: "mem_promoted",
      proposedText: "Rio suka kopi",
      proposedAt: T0,
      provenance: { sourceApp: "local" },
      trust: "LOCAL_AGENT",
      scope: "personal",
    });
    repo.promoteFromQuarantine(
      id,
      factInput({ id: "mem_promoted", sourceEpisodeIds: ["epi_source"] }),
      T1,
    );
