import { FlowNodeDefinitionSchema, WorkspaceIdSchema, type FlowNodeDefinition, type Timestamp } from "@ecorione/shared-schema";
...
const NodeDeclarationSyncSchema = z.object({
...
function nodeExecutePermission(db: HubDatabase): PermissionRow {
  const row = db.raw.prepare("SELECT permissions_json FROM capability_definitions WHERE id='node.execute'").get() as { permissions_json: string } | undefined;
  ...
  if (permission === undefined) throw new Error("Builtin permission node.execute belum tersedia.");
  return permission;
}

export function syncCoreNodeDeclarations(db: HubDatabase, workspaceId: string, definitions: readonly FlowNodeDefinition[], now: Timestamp): void {
  const permission = nodeExecutePermission(db);
  const tx = db.raw.transaction(() => {
    db.raw.prepare("DELETE FROM authority_declarations WHERE workspace_id=? AND subject_kind='node' AND subject_id LIKE 'core/%'").run(workspaceId);
    for (const definition of definitions) {
      db.raw.prepare(`INSERT INTO authority_declarations(
      ...
      ) VALUES(?,'node',?,'node.execute','node.execute',?,?,?,?,?,?,?)`).run(
        workspaceId,
        definition.id,
    ...
    db.raw.prepare(`DELETE FROM authority_grants
