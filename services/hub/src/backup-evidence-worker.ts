...
import { backupHubDatabase, restoreHubDatabase } from "./backup.js";
...
if (action === "backup") {
  if (!existsSync(source)) {
    console.log(JSON.stringify({ owner: "hub", action, status: "missing", source }));
  } else {
  ...
  if (ids.db === undefined) throw new Error("hub backup id missing");
  const target = resolve(restoreRoot, "hub/hub.db");
  if (target === source) throw new Error("isolated restore target resolves to active Hub DB");
  const receipt = restoreHubDatabase(backupRoot, ids.db, target, timestamp);
  const verification = inspect(target);
  if (verification.quickCheck !== "ok" || verification.foreignKeyViolations !== 0) {
    throw new Error(`restored Hub DB integrity failed: ${JSON.stringify(verification)}`);
  }
  console.log(
    JSON.stringify({ owner: "hub", action, status: "restored", target, receipt, verification }),
  );
} else {
