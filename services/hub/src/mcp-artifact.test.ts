...
describe("MCP memory_open → Artifact", () => {
  ...
  it("fails closed when Artifact redirects instead of following the owner redirect", async () => {
    let redirectedTargetHit = false;
    const artifact = createServer({ name: "artifact-redirect-test" });
    artifact.get<{ Params: { id: string } }>(
      "/v1/artifacts/:id/content",
      async (_req, reply) => reply.redirect("/redirect-target"),
    );
    artifact.get("/redirect-target", async () => {
    ...
    const address = artifact.server.address();
    if (address === null || typeof address === "string")
      throw new Error("Artifact redirect test address gagal.");
    ...
    const hub = buildHubServer(db, {
      ...
      rndUrl: "http://127.0.0.1:1",
      artifactUrl: `http://127.0.0.1:${String(address.port)}`,
      internalToken: "hub-internal-test-token",
    });
    const id = `art_${"b".repeat(64)}`;
    const response = await hub.inject({
      method: "POST",
      url: "/v1/mcp/memory/open",
      payload: {
