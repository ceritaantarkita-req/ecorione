...
export class ApprovalNotFoundError extends Error {
  constructor(id: string) {
    super(`Approval tidak ditemukan: ${id}.`);
    this.name = "ApprovalNotFoundError";
  }
}
export class ApprovalAlreadyDecidedError extends Error {
  constructor(id: string, status: string) {
    super(`Approval ${id} sudah diputuskan sebelumnya (status: ${status}).`);
...
export class RespondNotAllowedError extends Error {
  constructor() {
    super(
      'Decision "RESPOND" tidak boleh dipakai untuk approval aksi efek-samping. Pakai REJECT.',
    );
...
export class HubRepository {
  ...
  recordAuditEvent(input: RecordAuditEventInput): AuditEvent {
    ...
    this.db.raw
      .prepare(
        "INSERT INTO audit_events (id,ts,type,operation_id,module,detail,rule_id) VALUES (?,?,?,?,?,?,?)",
      )
      .run(
        id,
        input.now,
  ...
  createApproval(input: CreateApprovalInput): Approval {
    ...
    this.db.raw
      .prepare(
        "INSERT INTO approvals (operation_id,action_request,status,prompt,note,decided_by,decided_at,created_at) VALUES (?,?,'PENDING',?,NULL,NULL,NULL,?)",
      )
  ...
  ensureApproval(input: CreateApprovalInput): Approval {
  ...
  decideApproval(
    ...
    this.db.raw
      .prepare(
        "UPDATE approvals SET status=?,note=?,decided_by=?,decided_at=? WHERE operation_id=?",
      )
  ...
  getIdempotentResult<T>(key: string): IdempotentResult<T> | null {
    const row = this.db.raw
      .prepare("SELECT * FROM idempotent_results WHERE idempotency_key=?")
      .get(key) as IdempotentRow | undefined;
    ...
    return {
      ...
      completedAt: row.completed_at as Timestamp,
    };
  }
  saveIdempotentResult<T>(
    key: string,
    ...
    this.db.raw
      .prepare(
        "INSERT INTO idempotent_results (idempotency_key,operation_id,tool,result_json,completed_at) VALUES (?,?,?,?,?)",
      )
