...
export function bindHost(): string { return process.env.ECORIONE_ALLOW_REMOTE_BIND === "1" ? "0.0.0.0" : "127.0.0.1"; }
...
export function createServer(options: CreateServerOptions): FastifyInstance {
  ...
  app.setErrorHandler((err: unknown, req: FastifyRequest, reply: FastifyReply) => {
    if (err instanceof HttpError) return reply.code(err.statusCode).send({ ...errorBody(err), requestId: req.requestId });
    if (err instanceof ZodError) return reply.code(400).send({
      error: { type: "BAD_REQUEST", message: "Input tidak valid.", detail: { issues: err.issues.map((i) => ({ path: i.path.join("."), message: i.message })) } },
      requestId: req.requestId,
    });
    req.log.error({ err }, "unhandled error");
    return reply.code(500).send({ error: { type: "INTERNAL", message: "Kesalahan internal service." }, requestId: req.requestId });
  });
