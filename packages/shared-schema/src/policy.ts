...
export function autonomyExceeds(level: AutonomyLevel, ceiling: AutonomyLevel): boolean { return AUTONOMY_RANK[level] > AUTONOMY_RANK[ceiling]; }

export const ACTION_CLASS = ["READ", "REVERSIBLE_WRITE", "IRREVERSIBLE_WRITE", "SPEND", "EXTERNAL_SEND", "CREDENTIAL_ACCESS", "EXECUTE"] as const;
export type ActionClass = (typeof ACTION_CLASS)[number];
export const ActionClassSchema = z.enum(ACTION_CLASS);
const ALWAYS_GATED: ReadonlySet<ActionClass> = new Set(["IRREVERSIBLE_WRITE", "SPEND", "EXTERNAL_SEND", "CREDENTIAL_ACCESS"]);
export function alwaysRequiresApproval(cls: ActionClass): boolean { return ALWAYS_GATED.has(cls); }
...
export const ActionRequestSchema = z.object({
  ...
  if (value.actionClass !== "READ" && value.idempotencyKey === null) {
    ctx.addIssue({ code: "custom", path: ["idempotencyKey"], message: "Side effect wajib punya idempotencyKey (ADR-12)." });
  }
});
...
export const PolicyRuleSchema = z.object({ id: z.string().min(1).max(64), description: z.string().min(1).max(256), version: z.string().min(1).max(32) });
export type PolicyRule = z.infer<typeof PolicyRuleSchema>;

export const AUDIT_EVENT_TYPES = [
  "ACTION_REQUESTED", "POLICY_EVALUATED", "APPROVAL_REQUESTED", "APPROVAL_DECIDED",
  "ACTION_EXECUTED", "ACTION_FAILED", "ACTION_SKIPPED_IDEMPOTENT",
  "MEMORY_PROPOSED", "MEMORY_PROMOTED", "MEMORY_INVALIDATED", "MODEL_CALLED", "TRACE_WRITE_FAILED",
  "MCP_TOOL_CALLED",
] as const;
...
export function idempotencyPayload(req: Pick<ActionRequest, "module" | "tool" | "args">): string {
