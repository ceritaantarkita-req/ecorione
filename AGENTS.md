...
- off-host Backup & DR: **ACTIVE / CHECKPOINT 2 EXECUTION + CLEAN-HOST RECOVERY PATH**; checkpoint 1 is CLOSED / PASS at repository foundation, while real independent copy and total-host-loss recovery remain unproven;
...
**Off-host Backup & DR is the only active operational scope.** It is explicitly authorized as a separate infrastructure workstream and is not PCS-11, PE-09, Batch 13, production promotion, or a feature batch. Checkpoint 1 is CLOSED / PASS at the repository-foundation boundary. Checkpoint 2 adds current-revision export orchestration, retained export manifests, independent fetch/retrieval receipts, guarded exact-volume clean-host restore, application acceptance, and replacement-host reboot evidence. Do not claim real off-host protection or total-host-loss recovery until the runtime gates in `docs/offhost-dr-recovery.md` pass on an independent failure domain.
