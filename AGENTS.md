...
- off-host Backup & DR: **ACTIVE / RUNTIME EXECUTION PENDING**; checkpoints 1–3 are CLOSED / PASS at repository boundaries, while real independent copy and total-host-loss recovery remain unproven;
...
**Off-host Backup & DR is the only active operational scope.** It is explicitly authorized as a separate infrastructure workstream and is not PCS-11, PE-09, Batch 13, production promotion, or a feature batch. Checkpoints 1–3 are CLOSED / PASS at their repository boundaries. Checkpoint 3 added the clean replacement-host preflight plus loopback-only Caddy/security/MCP/Ops acceptance so total-host-loss recovery does not depend on the lost SumoPod Traefik/public edge. The active scope is now runtime execution: governed staging activation of the exact reviewed tooling, real independent off-host export, clean replacement-host retrieval/restore, semantic acceptance, and changed-boot-id reboot proof. Do not claim real off-host protection or total-host-loss recovery until the runtime gates in `docs/offhost-dr-recovery.md` pass on an independent failure domain.
