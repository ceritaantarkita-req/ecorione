...
describe("/api/projects/:id/sources/ingest-url", () => {
  it("forwards a validated HTTPS URL ingestion to Hub", async () => {
    hubPool
      .intercept({
        path: "/v1/projects/prj_finance/sources/ingest-url",
        method: "POST",
      })
      .reply(200, {
        operationId: "op_projecturlproxy001",
        ...
        artifact: {
          ...
          description: "URL snapshot: https://example.com/source.txt",
          mimeType: "text/plain",
          sizeBytes: 5,
          scope: "personal",
    ...
    const response = await POST(
      new Request("http://ai.local/api/projects/prj_finance/sources/ingest-url", {
        method: "POST",
  ...
  it("rejects non-HTTPS URL input before Hub", async () => {
    const response = await POST(
      new Request("http://ai.local/api/projects/prj_finance/sources/ingest-url", {
        method: "POST",
        headers: { "content-type": "application/json" },
