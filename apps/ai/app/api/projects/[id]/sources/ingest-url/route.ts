...
const BodySchema = ProjectUrlIngestRequestSchema.omit({ operationId: true });
...
export async function POST(
  ...
  if (!parsed.success) {
    return jsonError(400, "BAD_REQUEST", "Permintaan URL ingestion tidak valid.");
  }

  let upstream: Response;
  try {
    upstream = await fetch(
      `${hubUrl()}/v1/projects/${encodeURIComponent(projectId)}/sources/ingest-url`,
      {
        method: "POST",
        headers: headers(),
        body: JSON.stringify({
