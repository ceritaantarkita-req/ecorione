...
const BodySchema = ProjectMcpResourceIngestRequestSchema.omit({ operationId: true });
...
export async function POST(
  ...
  if (!parsed.success) {
    return jsonError(400, "BAD_REQUEST", "Permintaan MCP resource ingestion tidak valid.");
  }

  let upstream: Response;
  try {
    upstream = await fetch(
      `${hubUrl()}/v1/projects/${encodeURIComponent(projectId)}/sources/ingest-mcp-resource`,
      {
        method: "POST",
        headers: headers(),
        body: JSON.stringify({
