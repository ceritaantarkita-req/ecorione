...
describe("GET /api/ops", () => {
  ...
  it("satu owner gagal tetap menghasilkan snapshot eksplisit degraded, bukan melempar seluruh route", async () => {
    vi.stubGlobal(
      "fetch",
    ...
    const body = (await response.json()) as {
      healthy: boolean;
      services: Array<{ name: string; healthy: boolean; error: string | null }>;
    };
