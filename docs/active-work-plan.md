...
F6-E05 fixed runner OS labels: ACTIVE
```
...
| F6-E05 | Fixed GitHub-hosted runner OS labels + gate | **ACTIVE** | Replace mutable `*-latest` runner labels with fixed supported labels and reject regressions. |
...
F6-E05 = ACTIVE
next = fixed runner labels + repo-wide gate + tests -> exact-head CI/Product Eval -> guarded merge
