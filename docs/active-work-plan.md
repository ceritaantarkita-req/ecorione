...
**Current authorized scope: Session 2 — Restore Auto Deploy.** Controlled exact-SHA convergence has passed and `ECORIONE_STAGING_CD_ENABLED=1` has been restored. A governed workflow-dispatch deploy revalidated exact current main through the restricted SSH path. Do not start remaining HIGH audit fixes, Space/Project UX work, production promotion, or DR-2 runtime work before the final automatic post-merge proof closes Session 2.

### Session 2 — controlled convergence PASS / deploy path enabled / automatic proof pending
...
Repository variable `ECORIONE_STAGING_CD_ENABLED=1` was restored and Staging Deploy run `35999280717` / #743 passed through the restricted SSH path. The subsequent true automatic run `36000222819` / #751 reached deployment for exact new main but failed the first post-startup Operations sample because `space` was transiently unhealthy; rollback to known-good runtime passed. The bounded Operations-readiness retry is merged in PR #302, the installed privileged helper now matches reviewed main, and controlled convergence of `bd31ab7d64c0579f752fb225ed78b1dab94eed2a` is PASS. **Only remaining authorized work:** restore `ECORIONE_STAGING_CD_ENABLED=1`, merge this final checkpoint, and require the true automatic `workflow_run` deploy to pass full runtime validation for the resulting exact main SHA. Do not start HIGH audit fixes, Space/Project UX work, production promotion, or DR-2 runtime work before Session 2 closes. Evidence: [verification/staging-auto-deploy-restore-2026-09-24.md](verification/staging-auto-deploy-restore-2026-09-24.md).
