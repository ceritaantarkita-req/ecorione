...
Repository variable `ECORIONE_STAGING_CD_ENABLED=1` was restored and Staging Deploy run `35999280717` / #743 passed through the restricted SSH path. The subsequent true automatic run `36000222819` / #751 reached deployment for exact new main but failed the first post-startup Operations sample because `space` was transiently unhealthy; rollback to known-good runtime passed. **Only remaining authorized work:** land the bounded Operations-readiness retry, refresh the installed privileged helper from that reviewed source, re-run controlled convergence, then repeat the automatic post-merge proof. Do not start HIGH audit fixes, Space/Project UX work, production promotion, or DR-2 runtime work before Session 2 closes. Evidence: [verification/staging-auto-deploy-restore-2026-09-24.md](verification/staging-auto-deploy-restore-2026-09-24.md).
