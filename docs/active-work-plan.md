...
## Audit follow-up checkpoint — SESSION 10 / A-10 CLOSED / SAFE CHECKPOINT
...
**A-11 personal-workspace-first limitation is ACTIVE / BOUNDED.** The operator explicitly selected A-11 after the A-10 closure. This slice removes page/controller-owned `ws_personal` decisions from the browser product by introducing one validated browser Workspace context. Query `?workspace=<ws_...>` wins over persisted `ecorione.workspaceId`; invalid or absent candidates fall back to the canonical `DEFAULT_WORKSPACE_ID` for V1 compatibility. Ai, Projects, Work, Brain, Space, Flow, and Settings consume that shared context; Project/owner reads that depend on Workspace selection wait until the context is resolved. The scope does **not** create a Workspace directory/switcher, multi-user identity model, new service/database, new authorization semantics, native Google Drive integration, hosted spend, DR-2 work, or production cutover. Closure requires deterministic unit/source-contract coverage plus the normal exact-head gates and an exact-SHA staging acceptance.
