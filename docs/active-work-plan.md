...
Highest-priority audit findings are the latent fail-open direct-start remote-bind/auth configuration gap and incomplete bounded timeout coverage for selected internal HTTP owner calls. A separate concrete configuration bug points standalone Space at Ai fallback port `17029` instead of canonical Flow `17028`. Product-facing findings include the inert Projects `All` control, stale persisted Project selection, Project settings/source onboarding gaps, Schedule calendar/chat UX gaps, Brain scaling/richness gaps, frontend page concentration, and Compose readiness. The guarded Compose paths still require the internal token, and no current staging exposure is claimed. Full evidence: [verification/current-main-staging-audit-2026-09-24.md](verification/current-main-staging-audit-2026-09-24.md).
