...
| F6-E06 | Immutable Node toolchain + drift gate | **ACTIVE** | Replace mutable workflow Node major selector with centralized exact version and enforce cross-surface consistency. |
...
F6-E06 = ACTIVE
next = central exact Node version + workflow/container consistency gate + tests -> exact-head CI/Product Eval -> guarded merge
