...
F6-E05 fixed runner OS labels: IMPLEMENTED / IN REVIEW
```
...
| F6-E05 | Fixed GitHub-hosted runner OS labels + gate | **IMPLEMENTED / IN REVIEW** | Fixed Linux/Windows labels + repo-wide runner review + CI/release wiring; exact-head gates pending. |
...
next = exact-head CI/Product Eval + triggered workflow acceptance -> fix findings -> guarded merge -> closure
