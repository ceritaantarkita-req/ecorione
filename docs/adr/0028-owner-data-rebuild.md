...
3. **Dry-run mendahului execute.** Plan mencatat action, schema migration state, source digest, projection digest, integrity/orphan findings, normalization count, duplicate-derived-fact groups, dan plan digest. Execute menolak plan jika source atau projection berubah setelah dry-run.
...
8. **Rollback adalah projection rollback, bukan database replacement.** Snapshot source digest harus sama dengan current L0. Jika L0 sudah bertambah/berubah, rollback ditolak dan operator harus rebuild dari L0 terbaru.
