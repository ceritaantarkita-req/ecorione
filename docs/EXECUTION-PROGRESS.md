...
PCS-08 repository implementation is now in an active candidate branch. It adds `staging-deploy.yml`, a dedicated forced-command SSH boundary, exact-current-main verification, serialized host deployment, public/ops/exact-host post-deploy checks, release identity receipts, and prior-image/source runtime rollback. Deterministic source-contract coverage plus normal shell/release-security coverage are included. Real GitHub Environment secret configuration, host bootstrap, automatic deployment, and rollback evidence remain pending. Runbook: [staging-continuous-deployment.md](staging-continuous-deployment.md). Evidence candidate: [verification/pcs-08-repository-preparation-2026-09-20.md](verification/pcs-08-repository-preparation-2026-09-20.md).
