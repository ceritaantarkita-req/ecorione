...
- [offhost-dr-recovery.md](offhost-dr-recovery.md) — active off-host backup and total-host-loss recovery workstream; checkpoints 1–6 are closed at repository boundaries; checkpoint 7 adds marker-before-fetch retrieval enforcement; real runtime execution is pending.
...
- active operational scope — **Off-host Backup & DR / RUNTIME EXECUTION PENDING**; checkpoints 1–6 are CLOSED / PASS at repository boundaries, checkpoint 7 marker-before-fetch enforcement is under repository review, while real independent copy, retained-generation/timing evidence, and total-host-loss application recovery remain pending runtime evidence.
