...
- [offhost-dr-recovery.md](offhost-dr-recovery.md) — active off-host backup and total-host-loss recovery workstream; checkpoints 1–5 are closed at repository boundaries; checkpoint 6 adds immutable loss-marker timing provenance; real runtime execution is pending.
...
- active operational scope — **Off-host Backup & DR / RUNTIME EXECUTION PENDING**; checkpoints 1–5 are CLOSED / PASS at repository boundaries, checkpoint 6 loss-marker provenance is under repository review, while real independent copy, retained-generation/timing evidence, and total-host-loss application recovery remain pending runtime evidence.
