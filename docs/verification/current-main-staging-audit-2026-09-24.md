...
Remaining A-08 candidates are now limited to connector/resource hierarchy only where stable authorized canonical parent/child identities exist, and Core Memory representation only where Context exposes a stable canonical identity. Neither is implicitly authorized; A-09/A-10 remain separate until this remainder is selected or deferred.
