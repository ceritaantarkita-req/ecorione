...
**A-11 personal-workspace-first limitation is now ACTIVE at a bounded browser-context boundary.** Work starts from exact base `ff07de7a82f3481723ae245d0789c32dce60d824` on branch `agent/a11-browser-workspace-context-20260926`. The implementation centralizes browser Workspace selection instead of letting individual Ai product pages hardcode `ws_personal`; Personal remains the compatibility default, while a valid explicit/persisted Workspace can flow through existing owner APIs. No backend authority model, service ownership, persistence owner, scheduler, memory store, graph store, hosted-provider path, DR-2 scope, native Drive integration, or production gate is changed. A-11 remains open until exact-head verification, merge, and exact-SHA staging acceptance pass.
