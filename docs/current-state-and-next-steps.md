...
F6-E08 is **IMPLEMENTED / IN REVIEW**. `deploy/container-image-lock.json` is the authoritative reviewed identity map. Dockerfile Node plus governed Postgres/Caddy/Temporal compose images preserve readable tags while binding to the exact locked OCI SHA-256 digests. A deterministic lock-aware digest review is wired into normal CI and release-security acceptance; production-ops acceptance checks runtime surfaces against the same lock. The temporary registry resolver used in CI #1078 was removed after discovery. Closure now depends on exact-head CI/Product Eval and guarded merge.
