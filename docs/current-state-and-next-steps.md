...
The repository variable was restored to `ECORIONE_STAGING_CD_ENABLED=1`. Governed Staging Deploy run `35999280717` / #743 passed both gate and deploy jobs through the least-privilege SSH forced-command path and revalidated the already-recorded exact current main. The first true automatic post-merge run then exposed a deployment-acceptance readiness race: automatic run `36000222819` / #751 built and started exact main `a12094a45ee15339f4005e33bb30c2d27dc205bc`, passed public smoke, but the first authenticated Operations sample reported only `space` unhealthy immediately after startup. The helper correctly failed closed and rolled back to known-good `fad170645ba612b746453487dc97cc0e03cb05e7`. The bounded Operations-health retry fix merged in PR #302 as `bd31ab7d64c0579f752fb225ed78b1dab94eed2a`. Automatic CD was temporarily disabled, the privileged helper was refreshed from that exact reviewed main, and controlled convergence passed: public/MCP boundaries PASS, Operations healthy with no unhealthy services, exact-host identity matched, all 15 configured services were running, and disk headroom was `22.66 GiB`. Only the final true post-merge automatic proof remains; automatic closure is not yet claimed. Evidence-in-progress: [verification/staging-auto-deploy-restore-2026-09-24.md](verification/staging-auto-deploy-restore-2026-09-24.md).
