...
Marker-bound retrieval, isolated decrypt/Docker verification, clean replacement-host preflight, and guarded real project-volume restore now PASS. The exact 12 Compose-owned volumes were restored and fingerprint-verified on the clean replacement host, while project container count remains zero. The restore emitted a mode-controlled receipt path under `/var/lib/ecorione-dr`; exact-path receipt inspection is the immediate gate before standalone loopback startup. Next runtime gate: restore-receipt verification -> standalone loopback start/acceptance -> changed-boot-ID reboot evidence -> marker-bound final closure receipt. Until all of those pass, **total-host-loss recovery remains NOT YET PROVEN**.
