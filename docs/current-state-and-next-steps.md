...
The repository variable is now restored to `ECORIONE_STAGING_CD_ENABLED=1`. Governed Staging Deploy run `35999280717` / #743 passed both gate and deploy jobs through the least-privilege SSH forced-command path and revalidated the already-recorded exact current main, including private-edge smoke, MCP/OAuth behavior, authenticated Operations, exact-host identity, all 15 configured services, and `27.51 GiB` free disk. The remaining Session 2 proof is one true post-merge `workflow_run` deployment on a new reviewed main SHA. Evidence-in-progress: [verification/staging-auto-deploy-restore-2026-09-24.md](verification/staging-auto-deploy-restore-2026-09-24.md).
