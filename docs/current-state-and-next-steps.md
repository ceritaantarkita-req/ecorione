...
The active hardening branch builds the shared ECORIONE application image once, uses `--no-build` for Compose activation and rollback, requires release receipt == Git HEAD == active image, fully revalidates rollback, keeps only current+recorded rollback staging images, and targets 25 GiB post-deploy free space using only bounded BuildKit cleanup. Backup freshness per deployment remains a separately recorded deferred policy question rather than an implicit claim. Audit: [verification/deployment-pipeline-audit-2026-09-24.md](verification/deployment-pipeline-audit-2026-09-24.md).
