...
old = '''async function startTunnel(localUrl, label) {
...
new = '''async function startTunnel(localUrl, label) {
  const child = spawn(CLOUDFLARED_BIN, ["tunnel", "--no-autoupdate", "--url", localUrl], {
    stdio: ["ignore", "pipe", "pipe"],
  });
  const chunks = [];
...
async function startReadyTunnel(localUrl, label) {
  ...
  throw new Error(`Public HTTPS ${label} gagal setelah 2 tunnel attempts: ${failures.join(" | ")}`);
}
'''

if text.count(old) != 1:
    raise SystemExit(f"expected one tunnel function block, got {text.count(old)}")
text = text.replace(old, new, 1)

old = '''  oauthTunnel = await startTunnel(`http://${LOOPBACK}:${String(oauthPort)}`, "OAuth/JWKS");
  oauthPublicUrl = oauthTunnel.publicUrl;
  await waitForPublic(`${oauthPublicUrl}/healthz`, "OAuth/JWKS");
'''
new = '''  oauthTunnel = await startReadyTunnel(
    `http://${LOOPBACK}:${String(oauthPort)}`,
    "OAuth/JWKS",
  );
  oauthPublicUrl = oauthTunnel.publicUrl;
'''
if text.count(old) != 1:
    raise SystemExit(f"expected one OAuth call block, got {text.count(old)}")
text = text.replace(old, new, 1)

old = '''  syncTunnel = await startTunnel(`http://${LOOPBACK}:${String(syncPort)}`, "Sync MCP bridge");
  const syncPublicUrl = syncTunnel.publicUrl;
  await waitForPublic(`${syncPublicUrl}/healthz`, "Sync MCP bridge");
'''
new = '''  syncTunnel = await startReadyTunnel(
...
'''
if text.count(old) != 1:
    raise SystemExit(f"expected one Sync call block, got {text.count(old)}")
