...
import { reviewContainerImageDigests } from "./container-image-digest-review.mjs";
...
if (
  packageJson?.scripts?.["images:digest-review"] !==
  "node scripts/container-image-digest-review.mjs"
) {
...
try {
  const containerDigestReview = reviewContainerImageDigests();
  for (const finding of containerDigestReview.findings) {
    findings.push(`Container image digest drift: ${finding}`);
  }
} catch (error) {
  findings.push(
    `Container image digest review failed: ${error instanceof Error ? error.message : String(error)}`,
  );
}
