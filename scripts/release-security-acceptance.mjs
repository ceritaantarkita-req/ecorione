...
if (compose.includes("docker.sock")) findings.push("application baseline must not mount docker.sock");
if (!caddy.includes("/settings*") || !caddy.includes("basic_auth")) findings.push("settings must remain behind operator Basic Auth");
if (!server.includes("timingSafeEqual") || !server.includes("RATE_LIMITED")) findings.push("shared auth/rate-limit hardening missing");
if (!proxy.includes("const ALLOWED") || !proxy.includes('redirect: "error"')) findings.push("settings proxy path/redirect hardening missing");
if (!flow.includes("ECORIONE_FLOW_HTTP_HOST_ALLOWLIST") || !flow.includes('redirect: "error"')) findings.push("Flow SSRF boundary missing");
if (!mcpTypes.includes("allowInsecureLoopback") || !mcpTypes.includes("credentialRef")) findings.push("MCP transport credential/HTTPS schema missing");
if (!mcpSdk.includes("ECORIONE_MCP_STDIO_ALLOWLIST")) findings.push("MCP stdio command allowlist missing");
