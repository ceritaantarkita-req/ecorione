...
[[ -f "$OPS_FILE" && ! -L "$OPS_FILE" ]] || { echo "Missing or unsafe operator credential file" >&2; exit 1; }
[[ "$(stat -c '%a' "$OPS_FILE")" == "600" ]] || { echo "Operator credential file must be mode 600" >&2; exit 1; }

install -d -o root -g root -m 0755 "$STATE_DIR"
exec 9>"$LOCK_FILE"
flock -n 9 || { echo "Another ECORIONE staging deployment is already running" >&2; exit 1; }
...
owner_run_with_tag() {
  local image_tag="$1"
  shift
  sudo -u "$OWNER" -H env     ECORIONE_DEPLOY_ENV=deploy/staging.env     ECORIONE_COMPOSE_PROJECT=ecorione-staging     ECORIONE_COMPOSE_OVERLAY=deploy/compose.sumopod.yml     ECORIONE_EDGE_NETWORK="$ECORIONE_STAGING_EDGE_NETWORK"     ECORIONE_TRAEFIK_CERTRESOLVER="$ECORIONE_STAGING_CERTRESOLVER"     ECORIONE_IMAGE_TAG="$image_tag"     bash -c 'cd "$1"; shift; exec "$@"' _ "$REPO" "$@"
}
...
wait_for_services() {
  ...
  expected="$(owner_run_with_tag "$image_tag" docker compose     -p ecorione-staging     --env-file deploy/staging.env     -f deploy/compose.yml     -f deploy/compose.sumopod.yml     config --services | sort)"

  for _ in $(seq 1 60); do
    running="$(owner_run_with_tag "$image_tag" docker compose       -p ecorione-staging       --env-file deploy/staging.env       -f deploy/compose.yml       -f deploy/compose.sumopod.yml       ps --status running --services | sort)"
...
if ! owner_run_with_tag "$TARGET_TAG" bash scripts/self-host-upgrade.sh --apply "$TARGET_TAG"; then
  rollback "compose upgrade/build failed"
  exit 1
fi

if ! wait_for_services "$TARGET_TAG"; then
  rollback "not every configured service reached running state"
  exit 1
fi

if ! owner_run_with_tag "$TARGET_TAG" env   ECORIONE_PUBLIC_BASE_URL="$ECORIONE_STAGING_PUBLIC_BASE_URL"   node scripts/production-public-smoke.mjs; then
  rollback "public smoke failed"
  exit 1
fi

OPS_USER="$(sed -n 's/^username=//p' "$OPS_FILE")"
OPS_PASS="$(sed -n 's/^password=//p' "$OPS_FILE")"
[[ -n "$OPS_USER" && -n "$OPS_PASS" ]] || {
  rollback "operator credential file is malformed"
  exit 1
}

if ! owner_run_with_tag "$TARGET_TAG" env   ECORIONE_PUBLIC_BASE_URL="$ECORIONE_STAGING_PUBLIC_BASE_URL"   ECORIONE_OPS_USER="$OPS_USER"   ECORIONE_OPS_PASSWORD="$OPS_PASS"   node scripts/production-ops-snapshot.mjs; then
  unset OPS_USER OPS_PASS
  rollback "authenticated /api/ops snapshot failed"
  exit 1
fi
unset OPS_USER OPS_PASS

if ! owner_run_with_tag "$TARGET_TAG" env   ECORIONE_EXPECTED_SHA="$TARGET_SHA"   node scripts/staging-host-evidence.mjs; then
  rollback "exact-host evidence failed"
  exit 1
fi
